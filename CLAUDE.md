...
- **Styling is Tailwind v4 on generated CTM tokens.** `scripts/generate-theme.ts` reads the sibling `color-the-map` checkout's v2 token sources and emits the committed `src/styles/theme.generated.css` (auto-runs via `predev`/`prebuild`; no-ops in CI, which only validates the committed file via `npm run validate:theme`). Content-script surfaces get compiled CSS through `src/styles/shadow.css?inline` injected into shadow roots; the options page links `foundation.css`. Reusable component looks live in `src/ui/recipes.ts`. Never hand-copy CTM CSS — extend `scripts/theme/policy.ts` instead.
