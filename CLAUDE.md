...
- **MV3 content scripts cannot use ES module imports.** The plugin bundles each content script as a standalone IIFE automatically (any shared `src/` modules get inlined), while the background SW, popup, and options page build as ESM. One `vite.config.ts` — no second config, no manifest fan-out script. `vitest.config.ts` is kept separate so the build plugin doesn't run during unit tests.
